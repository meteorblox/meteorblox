...
type ExploreActivity = {
  ...
  motherlodes: Array<{ round: number; winningTile: number; addedDslvr?: number; balanceDslvr?: number; payoutDslvr: number; hit: boolean; transaction: string | null; timestamp: string | null; winnerType?: string; winnerAddress?: string | null; winnerCount?: number; deployedSui?: number; vaultedSui?: number; winningsSui?: number }>;
  ...
  audit: Array<{ round: number; expectedWinnerPoolSui: number; actualWinnerPoolSui: number; treasurySui: number; rewardsSui: number; opsSui: number; keeperSui: number; paidSui: number; paidDslvr: number; winnerClaims: number; status: "pass" | "mismatch" | "pending" }>;
...
export default function ExplorePage() {
  ...
  return <main className="explore-page">
    ...
    <section className="explore-activity"><h2>Verified activity</h2>
      ...
      {activityTab === "motherlodes" && <div className="motherlode-table"><p className="tab-description">Confirmed Motherload history retained from Sui Testnet.</p><div className="motherlode-row motherlode-head"><span>ROUND</span><span>TILE</span><span>DSLVR WINNER</span><span>WINNERS</span><span>DEPLOYED</span><span>VAULTED</span><span>WINNINGS</span><span>MOTHERLOAD</span><span>TIME</span></div>{(activity?.motherlodes ?? []).filter((item) => item.hit).map((item) => { const recent = activity?.rounds.find((candidate) => candidate.round === item.round); const round = { winnerType: item.winnerType ?? recent?.winnerType, winnerAddress: item.winnerAddress ?? recent?.winnerAddress, winnerCount: item.winnerCount ?? recent?.winnerCount, deployedSui: item.deployedSui ?? recent?.deployedSui, vaultedSui: item.vaultedSui ?? recent?.vaultedSui, winningsSui: item.winningsSui ?? recent?.winningsSui }; return <div className="motherlode-row" key={`${item.round}-${item.transaction}`}><strong>#{String(item.round).padStart(6, "0")}</strong><span>#{item.winningTile}</span><span>{round.winnerType === "split" ? <b className="winner-split">Split</b> : round.winnerType === "individual" ? <b className="winner-wallet" title={round.winnerAddress ?? undefined}>{shortAddress(round.winnerAddress)}</b> : <b className="winner-pending">Indexing</b>}</span><span>{round.winnerCount ?? "—"}</span><span className="activity-sui"><SuiDrop />{round.deployedSui?.toFixed(4) ?? "—"}</span><span className="activity-sui"><SuiDrop />{round.vaultedSui?.toFixed(4) ?? "—"}</span><strong className="activity-sui"><SuiDrop />{round.winningsSui?.toFixed(4) ?? "—"}</strong><strong className="motherlode-hit">{item.payoutDslvr.toFixed(4)} DSLVR</strong>{item.transaction ? <a href={`https://testnet.suivision.xyz/txblock/${item.transaction}`} target="_blank" rel="noreferrer">{dateLabel(item.timestamp)}</a> : <span>{dateLabel(item.timestamp)}</span>}</div>})}{!(activity?.motherlodes ?? []).some((item) => item.hit) && <p className="activity-empty">No Motherload hits indexed yet. The pool is still growing.</p>}</div>}
      {activityTab === "personal" && (!account ? <div className="personal-empty"><strong>Connect your wallet to view personal activity.</strong><span>Your deployments and winnings will appear here automatically.</span><Link href="/mine">Connect on Mine →</Link></div> : <div className="personal-table"><div className="personal-row personal-head"><span>ROUND</span><span>TILES</span><span>DEPLOYED</span><span>SUI WON</span><span>DSLVR WON</span><span>RESULT</span><span>TIME</span></div>{(activity?.personal ?? []).map((item) => <div className="personal-row" key={`${item.round}-${item.transaction}`}><strong>#{String(item.round).padStart(6, "0")}</strong><span>{item.tiles.map((tile) => `#${tile}`).join(", ")}</span><span className="activity-sui"><SuiDrop />{item.deployedSui.toFixed(4)}</span><span className="activity-sui"><SuiDrop />{item.winningsSui.toFixed(4)}</span><span>{item.dslvrWinnings.toFixed(4)} DSLVR</span><b className={item.won ? "personal-win" : "personal-played"}>{item.won ? "WON" : "PLAYED"}</b>{item.transaction ? <a href={`https://testnet.suivision.xyz/txblock/${item.transaction}`} target="_blank" rel="noreferrer">{dateLabel(item.timestamp)}</a> : <span>{dateLabel(item.timestamp)}</span>}</div>)}{!activity?.personal.length && <p className="activity-empty">No recent deployments found for {shortAddress(account.address)}.</p>}</div>)}
    </section>
    ...
    <section className="accounting-audit"><div className="audit-heading"><div><p>ON-CHAIN ACCOUNTING</p><h2>Round integrity audit</h2><span>Recomputes the 90/5/2/3 SUI allocation and reconciles winner SUI plus the 0.25 DSLVR round reward.</span></div><strong className={(activity?.auditSummary.mismatches ?? 0) ? "audit-bad" : "audit-good"}>{activity ? `${activity.auditSummary.passed}/${activity.auditSummary.checked} verified` : "Checking…"}</strong></div>
      <div className="audit-table"><div className="audit-row audit-head"><span>ROUND</span><span>WINNER POOL</span><span>FEES 5/2/2/1</span><span>PAID</span><span>STATUS</span></div>{(activity?.audit ?? []).map((item) => <div className="audit-row" key={item.round}><strong>#{String(item.round).padStart(6, "0")}</strong><span>{item.actualWinnerPoolSui.toFixed(6)} SUI</span><span>{item.treasurySui.toFixed(6)} / {item.rewardsSui.toFixed(6)} / {item.opsSui.toFixed(6)} / {item.keeperSui.toFixed(6)}</span><span>{item.paidSui.toFixed(6)} SUI · {item.paidDslvr.toFixed(6)} DSLVR</span><b className={`audit-${item.status}`}>{item.status === "pass" ? "VERIFIED" : item.status.toUpperCase()}</b></div>)}</div>
    ...
    <footer className="explore-footer"><span>SLVRBLOX · Sui Testnet</span><Link href="/mine">Back to mining grid →</Link></footer>
  </main>;
}

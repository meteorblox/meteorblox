...
const upgradeBasePackageId = packageId;
  ...
  return (
    <main className="app-shell">
      ...
      <section className="round-strip" aria-label="Current round">
        <div><small>ROUND</small><strong>{chainLoading ? "â€”" : `#${String(chainState?.round ?? 0).padStart(6, "0")}`}</strong></div><div><small>TIME LEFT</small><strong>{chainState?.settled ? "SETTLED" : `${seconds}s`}</strong></div>
        <div><small>DEPLOYED</small><strong className="deployed-total"><i className="round-sui-icon" aria-hidden="true"><svg viewBox="0 0 32 40"><path d="M16 1.8C13.3 5.5 4.2 16.5 4.2 23.9A11.8 11.8 0 0 0 16 35.8a11.8 11.8 0 0 0 11.8-11.9C27.8 16.5 18.7 5.5 16 1.8Zm0 29.6a7.5 7.5 0 0 1-7.5-7.5c0-3.7 4.2-10.2 7.5-14.7 3.3 4.5 7.5 11 7.5 14.7a7.5 7.5 0 0 1-7.5 7.5Z"/></svg></i>{chainLoading ? "â€”" : (chainState?.potSui ?? 0).toFixed(4)}</strong></div>
        <div><small><span className="dslvr-word">DSLVR</span> STRIKE</small><strong className="meteor-shower-total"><img className="round-slvr-core" src="/brand/dslvr-coin.png" alt="DSLVR" />{mtbxRoundReward.toFixed(2)}</strong></div>
      </section>
      ...
      {view === "mine" ? <div className="workspace">
        ...
        <div className="stake-hero"><p className="eyebrow">DSLVR BURIAL YIELD · TESTNET BETA</p><h1>Stake DSLVR.</h1><p>Earn a proportional share of the 10% of market buybacks returned to DSLVR stakers. The other 90% is permanently buried.</p></div>
        <div className="stake-metrics"><article><small>TOTAL DSLVR STAKED</small><strong>0.000000</strong><span>Vault deployment pending</span></article><article><small>BUYBACK RESERVE</small><strong>0.000000 SUI</strong><span>5% of game entries</span></article><article><small>DSLVR BURIED</small><strong>0.000000</strong><span>90% of completed buybacks</span></article></div>
        <div className="stake-layout"><article className="stake-card"><div className="stake-tabs"><button className={stakeMode === "stake" ? "active" : ""} onClick={() => setStakeMode("stake")}>Stake</button><button className={stakeMode === "unstake" ? "active" : ""} onClick={() => setStakeMode("unstake")}>Unstake</button></div><div className="stake-balance"><span>{stakeMode === "stake" ? "AVAILABLE" : "STAKED"}</span><strong>0.000000 DSLVR</strong></div><label htmlFor="stake-amount">Amount</label><div className="stake-input"><input id="stake-amount" inputMode="decimal" placeholder="0.00" value={stakeAmount} onChange={(event) => setStakeAmount(event.target.value.replace(/[^0-9.]/g, ""))} /><span>DSLVR</span><button type="button" onClick={() => setStakeAmount("0")}>MAX</button></div><dl><div><dt>Lock period</dt><dd>None</dd></div><div><dt>Withdrawal</dt><dd>Any time</dd></div><div><dt>Yield source</dt><dd>10% of DSLVR buybacks</dd></div></dl><button className="deploy stake-submit" disabled>{stakeMode === "stake" ? "Staking vault deployment pending" : "No DSLVR currently staked"}</button></article><aside className="stake-position"><div className="position-orb"><img src="/brand/dslvr-coin.png" alt="DSLVR" /></div><small>YOUR POSITION</small><strong>0.000000 DSLVR</strong><span>0.00% of staking pool</span><div className="yield-box"><small>CLAIMABLE BUYBACK YIELD</small><strong>0.000000 DSLVR</strong><button disabled>Claim rewards</button></div></aside></div>
        <article className="burial-flow"><div><span>5%</span><small>SUI BUYBACK RESERVE</small></div><i>→</i><div><span>90%</span><small>DSLVR BURIED</small></div><i>+</i><div><span>10%</span><small>STAKER YIELD</small></div></article><p className="stake-warning">Testnet preview. No staking transaction or market buyback is active until the audited vault and DSLVR/SUI liquidity pool are deployed.</p><button className="back-link" onClick={() => setView("mine")}>Back to mining grid</button>
      </section>}
      ...
      {connectOpen && <div className="connect-backdrop" role="presentation" onMouseDown={() => setConnectOpen(false)}><section className="connect-card" role="dialog" aria-modal="true" aria-labelledby="connect-title" onMouseDown={(event) => event.stopPropagation()}>
        <button className="connect-close" aria-label="Close sign in" onClick={() => setConnectOpen(false)}>Ã—</button><span className="connect-orbit" aria-hidden="true"><i /></span>
      ...
      {accountOpen && currentAccount && <div className="account-backdrop" role="presentation" onMouseDown={() => setAccountOpen(false)}><aside className="account-drawer" role="dialog" aria-modal="true" aria-labelledby="account-title" onMouseDown={(event) => event.stopPropagation()}>
        <button className="account-close" aria-label="Close wallet panel" onClick={() => setAccountOpen(false)}>Ã—</button>
        ...
        <div className="username-editor"><label htmlFor="wallet-username">Username</label><div><input id="wallet-username" value={usernameDraft} maxLength={20} placeholder="Create username" onChange={(event) => setUsernameDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") saveUsername(); }} /><button onClick={saveUsername}>Save</button></div><small>3â€“20 letters, numbers, _ or - Â· saved on this device</small></div>
        ...
        <a className="account-explorer" href={`https://suiscan.xyz/testnet/account/${currentAccount.address}`} target="_blank" rel="noreferrer">View wallet on SuiScan â†—</a>

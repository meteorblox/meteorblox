...
export default function Home() {
  ...
  async function publishCleanTestnetPackage() {
    ...
    setNotice("Waiting for owner approval to publish a clean METEORBLOX Testnet deploymentâ€¦");
    try {
      const result = await executeWithSlush(transaction);
  ...
  async function claimRoundWinnings() {
    ...
    setRoundAction(true); setNotice("Waiting for wallet approval to claim SUI and award MTBXâ€¦");
    try {
      const result = await executeWithSlush(transaction);
    ...
    <main className="app-shell">
      <header className="topbar">
        ...
        <div className="top-actions">
          ...
          <span className="network"><i /> Sui Testnet</span><button className="wallet" onClick={openAccountDrawer}>{currentAccount ? `${currentAccount.address.slice(0, 6)}â€¦${currentAccount.address.slice(-4)}` : "Sign in"}</button>
        </div>
      </header>
      ...
      {view === "mine" ? <div className="workspace">
        <section className="mine-panel">
          <div className="section-heading"><div><p>LIVE GRID Â· DEMO</p><h1>Choose your impact zone.</h1></div>
          ...
          <div className="grid" aria-label="Mining grid">
            {tiles.map((tile) => {
              ...
              return <button key={tile} className={isSelected ? "tile selected" : "tile"} aria-label={`Block ${tile}, ${previewAmount.toFixed(3)} SUI`} aria-pressed={isSelected} onClick={() => toggleTile(tile)}><span className="tile-number">{tile}</span>{isSelected && <span className="tile-check" aria-hidden="true">âœ“</span>}<span className="meteor" aria-hidden="true"><i /><b /></span><span className="tile-balance"><i className="sui-icon" aria-hidden="true"><span /></i><strong>{previewAmount.toFixed(3)}</strong></span></button>;
            })}
          </div>
        </section>
        ...
        <aside className="entry-panel">
          ...
          <div className="rounds-control"><div className="rounds-label"><label htmlFor="rounds">Rounds</label><small>Repeat selected tiles</small></div><div className="rounds-stepper"><button aria-label="Remove one round" onClick={() => setRounds((value) => Math.max(1, value - 1))}>âˆ’</button><input id="rounds" type="number" min="1" inputMode="numeric" value={rounds} onChange={(event) => setRounds(Math.max(1, Math.floor(Number(event.target.value) || 1)))} /><button aria-label="Add one round" onClick={() => setRounds((value) => value + 1)}>+</button></div></div>
          ...
          <button className="rewards-link" onClick={() => setView("rewards")}><span><small>YOUR ON-CHAIN REWARDS</small><strong>{((chainState?.unrefinedMtbx ?? 0) + (chainState?.refinedMtbx ?? 0) + (chainState?.estimatedMtbxWinnings ?? 0)).toFixed(6)} MTBX + {(chainState?.estimatedSuiWinnings ?? 0).toFixed(6)} SUI</strong></span><b>View &amp; claim â†’</b></button>
      ...
      <footer><p><strong>$MTBX</strong> Â· Digital rare metal on Sui</p><nav aria-label="Footer"><a href="#how">How it works</a><a href="#token">Token</a><a href="#faq">FAQ</a></nav></footer>

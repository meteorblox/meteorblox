  ...
  async function closeEmptyRound() {
    ...
    transaction.moveCall({ target: `${packageId}::game::close_empty_round`, arguments: [transaction.object(gameId), transaction.object(suiClockId)] …1220 tokens truncated…2-3.37-1.22-3.37-1.22-.45-1.18-1.11-1.49-1.11-1.49-.91-.64.07-.63.07-.63 1 .07 1.53 1.06 1.53 1.06.9 1.56 2.35 1.11 2.92.85.09-.66.35-1.11.64-1.37-2.22-.26-4.56-1.14-4.56-5.06 0-1.12.39-2.03 1.03-2.75-.1-.26-.45-1.3.1-2.71 0 0 .84-.28 2.75 1.05A9.35 9.35 0 0 1 12 6.93c.85 0 1.7.12 2.5.35 1.91-1.33 2.75-1.05 2.75-1.05.55 1.41.2 2.45.1 2.71.64.72 1.03 1.63 1.03 2.75 0 3.93-2.34 4.79-4.57 5.05.36.32.68.94.68 1.9l-.01 2.81c0 .27.18.59.69.49A10.23 10.23 0 0 0 22 12.23C22 6.58 17.52 2 12 2Z" clipRule="evenodd" /></svg></button>
      ...
      {view === "mine" ? <div className="workspace">
        ...
        <article className="claim-card testnet-publish"><small>OWNER TESTNET UPGRADE</small><h2>Enable reliable 60-second rounds</h2><p>Install the latest tested package, then activate the shared autoplay registry once. The owner wallet is required.</p><button className="deploy" disabled={upgradingPackage} onClick={upgradeTestnetPackage}>{upgradingPackage ? "Waiting for Slush approval..." : "Upgrade Testnet"}</button><button className="deploy" disabled>Rewards Ledger active</button>{currentAccount?.address.toLowerCase() === testnetOwner && !autoplayRegistryId && <button className="deploy" disabled={creatingAutoplayRegistry} onClick={createAutoplayRegistry}>{creatingAutoplayRegistry ? "Waiting for Slush approval..." : "Create Autoplay Registry"}</button>}{currentAccount?.address.toLowerCase() === testnetOwner && !chainState?.settled && seconds === 0 && chainState?.potSui === 0 && <button className="deploy" disabled={roundAction} onClick={closeEmptyRound}>{roundAction ? "Waiting for Slush approval..." : "Close current empty round"}</button>}</article>

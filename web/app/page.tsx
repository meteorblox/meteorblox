...
const continuousPackageId = "0x92ced0ea9bd54cffec3e6472c2c9b235a06b4a5ec70092c41cebfa2cd2e54d16";
...
export default function Home() {
  ...
  async function …186 tokens truncated…ng for wallet approval to ${early ? "withdraw" : "claim"} MTBXâ€¦`);
    try {
      const result = await executeWithSlush(transaction);
  ...
  return (
    <main className="app-shell">
      ...
      {view === "mine" ? <div className="workspace">
        ...
        {currentAccount?.address.toLowerCase() === testnetOwner && <article className="claim-card testnet-publish"><small>OWNER TESTNET UPGRADE</small><h2>Enable reliable 60-second rounds</h2><p>Install the tested empty-round rollover fix, then keep the existing Rewards Ledger.</p><button className="deploy" disabled={upgradingPackage} onClick={upgradeTestnetPackage}>{upgradingPackage ? "Waiting for Slush approval..." : "Upgrade Testnet"}</button><button className="deploy" disabled>Rewards Ledger active</button>{!chainState?.settled && seconds === 0 && chainState?.potSui === 0 && <button className="deploy" disabled={roundAction} onClick={closeEmptyRound}>{roundAction ? "Waiting for Slush approval..." : "Close current empty round"}</button>}</article>}

  ...
  async function mintRehearsalTestUsdc() {
    ...
    transaction.moveCall({
      ...
      arguments: [
        ...
        transaction.pure.u64(5_000_000),
        transaction.pure.address(testnetOwner),
      ],
    });
    setRoundAction(true);
    setNotice("Waiting for approval to mint exactly 5 valueless tUSDC on Testnet...");
    try {
      const result = await executeWithSlush(transaction);
      if (result) setNotice(`Minted 5 mock tUSDC on Testnet. Transaction: ${result.digest}`);
    } catch (error) {
  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        ...
        <nav className="main-nav" aria-label="Main navigation"><button className={view === "mine" ? "active" : ""} onClick={() => setView("mine")}>Mine</button><button className={view === "rewards" ? "active" : ""} onClick={() => setView("rewards")}>Rewards</button><button className={view === "stake" ? "active" : ""} onClick={() => setView("stake")}>Stake</button><a href="/explore">Explore</a><a href="https://sale.slvrblox.com">Presale</a></nav>
      ...
      <section className="round-strip" aria-label="Current round">
        ...
        <div><small><span className="dslvr-word">DSLVR</span> MOTHERLOAD</small><strong className="meteor-shower-total"><img className="round-slvr-core" src="/brand/dslvr-coin.png" alt="DSLVR" />{mtbxRoundReward.toFixed(2)}</strong></div>
      </section>

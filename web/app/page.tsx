  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        ...
        <nav className="main-nav" aria-label="Main navigation"><button className={view === "mine" ? "active" : ""} onClick={() => setView("mine")}>Mine</button><button className={view === "rewards" ? "active" : ""} onClick={() => setView("rewards")}>Rewards</button><button className={view === "stake" ? "active" : ""} onClick={() => setView("stake")}>Stake</button><Link href="/explore">Explore</Link><a href="https://sale.slvrblox.com">Presale</a></nav>
      ...
      {view !== "mine" && <section className="round-strip" aria-label="Current round">
        ...
        <div><small><span className="dslvr-word">DSLVR</span> ROUND REWARD</small><strong className="meteor-shower-total"><img className="round-slvr-core" src="/brand/dslvr-coin.png" alt="DSLVR" />{mtbxRoundReward.toFixed(2)}</strong></div>
      </section>}

      {view !== "mine" && <section className={lastRoundOpen ? "last-round open" : "last-round"} aria-label="Last settled round">
      ...
      {view === "mine" ? <div className="workspace">
        ...
        <aside className="entry-panel compact-entry-panel">
          ...
          <dl className="compact-summary"><div><dt>Deployment</dt><dd>{total.toFixed(4)} SUI</dd></div></dl>

...
export default function Home() {
  ...
  async function upgradeTestnetPackage() {
    ...
    setNotice("Waiting for the owner wallet to approve the seven-day gradual-refinery Testnet upgrade...");
    try {
      const result = await executeWithSlush(transaction);
      if (result) { setNotice(`SLVRBLOX seven-day gradual-refinery upgrade completed. Transaction: ${result.digest}`); await refreshChainState(); }
    } catch (error) {
  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        ...
        <nav className="main-nav" aria-label="Main navigation"><button className={view === "mine" ? "active" : ""} onClick={() => setView("mine")}>Mine</button><button className={view === "rewards" ? "active" : ""} onClick={() => setView("rewards")}>Rewards</button><button className={view === "stake" ? "active" : ""} onClick={() => setView("stake")}>Stake</button><Link href="/explore">Explore</Link><Link href="/airdrop">Airdrop</Link><Link href="/chat">Chat</Link><a href="https://sale.slvrblox.com">Presale</a></nav>
      ...
      {view === "mine" ? <div className="workspace">
        ...
        {currentAccount?.address.toLowerCase() === testnetOwner && Number(chainState?.upgradeCap?.version ?? 0) < 7 && <article className="claim-card testnet-publish"><small>OWNER TESTNET UPGRADE</small><h2>Activate gradual DSLVR refining</h2><p>The live package is still on version {chainState?.upgradeCap?.version ?? "—"}. This tested upgrade makes new DSLVR rewards unlock continuously over seven days. Players can claim the refined portion anytime, while a 10% penalty applies only when withdrawing the remaining unrefined amount early. Existing positions, balances, and game objects are preserved.</p><button className="deploy" disabled={upgradingPackage} onClick={upgradeTestnetPackage}>{upgradingPackage ? "Waiting for wallet approval..." : "Upgrade Testnet package to version 7"}</button></article>}

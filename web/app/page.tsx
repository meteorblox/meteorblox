...
const rewardCapId = "0x1e2338d226d0bbefb3430f7912fe50dd535c12d1b733249019f93ff4c8678c17";
    ...
    <main className="app-shell">
      ...
      {view === "mine" ? <div className="workspace">
        ...
        <div className="reward-assets">
          <article className="claim-card refining-card"><div className="claim-icon">S</div><small>DSLVR REFINERY</small><div className="refinery-balances"><div><span>UNREFINED</span><strong>{(chainState?.unrefinedMtbx ?? 0).toFixed(6)} DSLVR</strong></div><div><span>REFINED</span><strong>{(chainState?.refinedMtbx ?? 0).toFixed(6)} DSLVR</strong></div></div><div className="refine-time"><span>Next maturity</span><strong>{chainState?.nextMaturityMs ? "Within 24h" : "â€”"}</strong></div><div className="refine-track" aria-label="DSLVR refining period"><i /></div><p className="refine-copy">DSLVR becomes fully transferable after 24 hours. Early withdrawal mints 90% and permanently forfeits 10%.</p><button className="deploy" disabled={roundAction || !(chainState?.refinedPositions)} onClick={() => claimMtbx(false)}>Claim refined DSLVR</button><button className="early-withdraw" disabled={roundAction || !(chainState?.unrefinedPositions)} onClick={() => claimMtbx(true)}>Withdraw unrefined early</button><p className="penalty-copy"><strong>10% penalty</strong> applies only to early withdrawal.</p></article>

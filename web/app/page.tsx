...
type ChainState = {
  ...
  ledgerSui: number;
  autoplayPlans: Array<{ planId: number; roundsRemaining: number; tileCount: number; amountPerTileSui: number; fundedSui: number; lastRoundPlayed: number }>;
    ...
    <main className="app-shell">
      ...
      {view === "mine" ? <div className="workspace">
        ...
        <aside className="entry-panel">
          ...
          <dl className="summary"><div><dt>Selected tiles</dt><dd>{selected.length}</dd></div><div><dt>Rounds</dt><dd>{rounds}</dd></div><div><dt>Per round</dt><dd>{(Number(amount) * selected.length || 0).toFixed(4)} SUI</dd></div><div><dt>Total deployment</dt><dd>{total.toFixed(4)} SUI</dd></div><div><dt>Winning reward</dt><dd>{mtbxRoundReward.toFixed(2)} MTBX + SUI</dd></div><div><dt>Autoplay left</dt><dd>{(chainState?.autoplayPlans ?? []).reduce((sum, plan) => sum + plan.roundsRemaining, 0)} rounds</dd></div></dl>

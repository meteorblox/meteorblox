...
const upgradeBasePackageId = "0x253e18bab31d507c276816e88f1a7c7f94045e9ad2913681a9e8dba98b20c406";
  ...
  return (
    <main className="app-shell">
      ...
      {view === "mine" ? <div className="workspace">
        ...
        {currentAccount?.address.toLowerCase() === testnetOwner && <article className="claim-card testnet-publish"><small>OWNER TESTNET UPGRADE</small><h2>Enable continuous 60-second rounds</h2><p>Step 1 upgrades the current Testnet package. Send us its transaction ID so the newly published package can be verified and bound before Step 2.</p><button className="deploy" disabled={upgradingPackage || creatingLedger} onClick={upgradeTestnetPackage}>{upgradingPackage ? "Waiting for Slush approval..." : "Step 1: Upgrade continuous engine"}</button><button className="deploy" disabled title="Enabled after the Step 1 package ID is verified">Step 2: Available after Step 1</button>{!chainState?.settled && seconds === 0 && chainState?.potSui === 0 && <button className="deploy" disabled={roundAction} onClick={closeEmptyRound}>{roundAction ? "Waiting for Slush approval..." : "Close current empty round"}</button>}</article>}

  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        ...
        <nav className="main-nav" aria-label="Main navigation"><Link className={view === "mine" ? "active" : ""} style={view === "mine" ? { borderColor: "#6fbcf0" } : undefined} href="/mine" onClick={() => { setView("mine"); setNotice(""); }}>Mine</Link><button className={view === "rewards" ? "active" : ""} onClick={() => setView("rewards")}>Rewards</button><a className={view === "stake" ? "active" : ""} style={view === "stake" ? { borderColor: "#6fbcf0" } : undefined} href="/stake">Stake</a><Link href="/explore">Explore</Link><Link href="/sentinel">Sentinel</Link><Link href="/airdrop">Airdrop</Link></nav>
      ...
      {view === "mine" ? <div className="workspace">
        ...
        <div className="stake-simple">
          ...
          <article className="stake-card">
            ...
            <button className="deploy stake-submit" disabled={Number(chainState?.upgradeCap?.version ?? 0) < 13 || submittingStake || !stakeAmount || (stakeMode === "unstake" && (stakingState.userStakedDslvr <= 0 || stakingState.unlockAtMs > Date.now()))} onClick={submitStakeAction}>{Number(chainState?.upgradeCap?.version ?? 0) < 13 ? "Staking upgrade pending" : stakeMode === "unstake" && stakingState.unlockAtMs > Date.now() ? "Stake locked" : submittingStake ? "Waiting for wallet approval..." : stakeMode === "stake" ? "Stake DSLVR" : "Unstake DSLVR"}</button>
          </article>

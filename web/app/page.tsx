...
export default function Home() {
  ...
  async function upgradeTestnetPackage() {
    ...
    setNotice("Waiting for the owner wallet to approve the Testnet recovery upgrade…");
    try {
      const result = await executeWithSlush(transaction);
    ...
    <main className="app-shell">
      ...
      {view === "mine" ? <div className="workspace">
        <section className="mine-panel">
          <div className="section-heading"><div><p>LIVE GRID · DEMO</p><h1>Choose your impact zone.</h1></div>
        ...
        <aside className="entry-panel">
          ...
          <div className="rounds-control"><div className="rounds-label"><label htmlFor="rounds">Rounds</label><small>Repeat selected tiles</small></div><div className="rounds-stepper"><button aria-label="Remove one round" onClick={() => setRounds((value) => Math.max(1, value - 1))}>−</button><input id="rounds" type="number" min="1" inputMode="numeric" value={rounds} onChange={(event) => setRounds(Math.max(1, Math.floor(Number(event.target.value) || 1)))} /><button aria-label="Add one round" onClick={() => setRounds((value) => value + 1)}>+</button></div></div>
          ...
          <button className="rewards-link" onClick={() => setView("rewards")}><span><small>YOUR ON-CHAIN REWARDS</small><strong>{((chainState?.unrefinedMtbx ?? 0) + (chainState?.refinedMtbx ?? 0) + (chainState?.estimatedMtbxWinnings ?? 0)).toFixed(6)} MTBX + {(chainState?.estimatedSuiWinnings ?? 0).toFixed(6)} SUI</strong></span><b>View &amp; claim →</b></button>
      ...
      <footer><p><strong>$MTBX</strong> · Digital rare metal on Sui</p><nav aria-label="Footer"><a href="#how">How it works</a><a href="#token">Token</a><a href="#faq">FAQ</a></nav></footer>

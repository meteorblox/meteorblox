...
type GameStatus = {
  ...
  keeperHealth: { updatedAt: number; lastSuccessAt: number; lastAutoplayAt: number; consecutiveFailures: number; lastError: string } | null;
  globalAutoplay: { activePlans: number; pendingPlans: number; remainingRounds: number; batchSize: number; packageVersion: number; carriedOverPlans: number; oldestPlanLag: number };
...
export default function StatusPage() {
  ...
  const alerts = useMemo(() => {
    ...
    if ((state.game?.globalAutoplay.carriedOverPlans ?? 0) > 0) found.push(`${state.game?.globalAutoplay.carriedOverPlans} autoplay plans carried over from an earlier round`);
    if (state.game && state.game.remainingMs <= 10_000 && state.game.globalAutoplay.pendingPlans > 0) found.push(`${state.game.globalAutoplay.pendingPlans} autoplay plans are still pending near round close`);
    if ((state.game?.keeperHealth?.consecutiveFailures ?? 0) >= 3) found.push(`Keeper has failed ${state.game?.keeperHealth?.consecutiveFailures} consecutive cycles: ${state.game?.keeperHealth?.lastError}`);
    if (state.game?.keeperHealth && Date.now() - state.game.keeperHealth.updatedAt > 60_000) found.push("Keeper heartbeat is more than 60 seconds old");
    return found;
  }, [state]);
  ...
  return <main className="status-page">
    ...
    <section className="status-grid">
      ...
      <article><small>KEEPER GAS</small><strong>{state.game ? `${fmt(state.game.keeperSui, 3)} SUI` : "—"}</strong><dl><div><dt>Funding</dt><dd className={state.game?.keeperLow ? "bad" : "good"}>{state.game?.keeperLow ? "Low" : "Healthy"}</dd></div><div><dt>Heartbeat</dt><dd className={state.game?.keeperHealth && Date.now() - state.game.keeperHealth.updatedAt <= 60_000 ? "good" : "bad"}>{state.game?.keeperHealth ? `${Math.max(0, Math.round((Date.now() - state.game.keeperHealth.updatedAt) / 1000))}s ago` : "Unavailable"}</dd></div><div><dt>Consecutive failures</dt><dd className={state.game?.keeperHealth?.consecutiveFailures ? "bad" : "good"}>{state.game?.keeperHealth?.consecutiveFailures ?? "—"}</dd></div><div><dt>Email alerts</dt><dd className={state.game?.alertsConfigured ? "good" : "bad"}>{state.game?.alertsConfigured ? "Enabled" : "Setup needed"}</dd></div><div><dt>Network</dt><dd>Testnet</dd></div></dl></article>
      <article><small>AUTOPLAY BATCHES</small><strong>{state.game ? `${state.game.globalAutoplay.activePlans} active` : "—"}</strong><dl><div><dt>Pending this round</dt><dd className={state.game?.globalAutoplay.pendingPlans ? "bad" : "good"}>{state.game?.globalAutoplay.pendingPlans ?? "—"}</dd></div><div><dt>Carried over</dt><dd className={state.game?.globalAutoplay.carriedOverPlans ? "bad" : "good"}>{state.game?.globalAutoplay.carriedOverPlans ?? "—"}</dd></div><div><dt>Oldest plan lag</dt><dd className={(state.game?.globalAutoplay.oldestPlanLag ?? 0) > 1 ? "bad" : "good"}>{state.game ? `${state.game.globalAutoplay.oldestPlanLag} rounds` : "—"}</dd></div><div><dt>Remaining plan-rounds</dt><dd>{state.game?.globalAutoplay.remainingRounds ?? "—"}</dd></div><div><dt>Plans per batch</dt><dd>{state.game?.globalAutoplay.batchSize ?? "—"}</dd></div><div><dt>Contract version</dt><dd className={(state.game?.globalAutoplay.packageVersion ?? 0) >= 9 ? "good" : "bad"}>v{state.game?.globalAutoplay.packageVersion ?? "—"}</dd></div></dl></article>
